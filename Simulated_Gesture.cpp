...
void TouchTask(void *parameter) {
  ...
  while(1){
    ...
    else if(PWR_KEY_State == DoubleClick){          
      current_obj_sequence = 0;   
      Old_Button = 0;
      Page_switching(false);
      if(current_obj_sequence == 0)
        current_obj_sequence = Page_Number;
      current_obj_sequence --;
      lv_obj_t *screen = lv_scr_act();
      lv_obj_invalidate(screen);
    }                                  
...
void Add_Style_Label(lv_obj_t *Label_obj, lv_style_t * Label_style){
  if(Label_obj){
    ...
    lv_area_t coords;                                 
    lv_obj_get_coords(Label_obj, &coords);
    if (coords.y2 > LCD_HEIGHT*7/10) {   
      scroll_up_fixed(Page_panel[current_Page_sequence],coords.y2 - (LCD_HEIGHT*7/10));
    }
    else if (coords.y1 < LCD_HEIGHT*3/10) {   
      scroll_up_fixed(Page_panel[current_Page_sequence],coords.y1 - (LCD_HEIGHT*3/10));
    }
